...
def html_retards_10min(production_chains: pd.DataFrame) -> str:
    ...
    # DÉDUPLICATION : Grouper par Poste + Nom + Référence pour éviter les doublons
    # (chaque opération peut avoir plusieurs lignes pour différentes personnes/semaines)
    ...
    retard_moyen = retards["Ecart_min"].mean()
    retard_max = retards["Ecart_min"].max()
    
    # Construction HTML manuelle (rouge) avec statistiques
    html = f"""
<h2 style="color:red;">⚠️ Postes avec un retard supérieur à 10 minutes</h2>
<div style="background-color:#fff3cd;padding:15px;border-left:4px solid #ff6b6b;margin-bottom:20px;border-radius:5px;">
    <p style="margin:5px 0;"><strong>📊 Statistiques :</strong></p>
    <p style="margin:5px 0;">• Nombre d'opérations en retard : <strong>{nb_retards}</strong></p>
    <p style="margin:5px 0;">• Retard moyen : <strong>{retard_moyen:.1f} min</strong></p>
    <p style="margin:5px 0;">• Retard maximum : <strong>{retard_max:.1f} min</strong></p>
</div>
...
<td>{row.get('Référence', '')}</td>
    ...
    # Ajouter un graphique à barres pour visualiser les retards
    try:
        # Prendre les 15 plus gros retards pour le graphique
        top_retards = retards.nlargest(15, "Ecart_min").copy()
        top_retards["Label"] = top_retards["Poste"].astype(str) + " - " + top_retards["Nom"].astype(str)
        ...
        fig = px.bar(
            ...
            y="Label",
            orientation="h",
            labels={"Ecart_min": "Retard (minutes)", "Label": "Poste - Opération"},
            title="🔴 Top 15 des retards les plus importants",
            color="Ecart_min",
            color_continuous_scale=["#ffd43b", "#ff6b6b", "#c92a2a"]
        )
        fig.update_layout(
...
# ======================================================
# 6. COÛTS PAR ÉTAPE (HTML) - FONCTION SUPPRIMEE (DOUBLON)
# La fonction correcte est plus bas dans le fichier (ligne ~1342)
# ======================================================
    ...
    cout_total_global = summary["Cout_total"].sum()
    cout_pieces_global = summary["Cout_pieces"].sum()
    cout_mo_global = summary["Cout_main_oeuvre"].sum()
    nb_etapes = len(summary)
...
<h2>💰 Coûts par Étape de production</h2>
...
<div class="stats-box">
    ...
    <div class="stats-grid">
        <div class="stat-item">
            ...
            <div class="stat-value">{nb_etapes}</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">Coût total global</div>
            <div class="stat-value">{cout_total_global:,.0f} €</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">Coût pièces</div>
            <div class="stat-value">{cout_pieces_global:,.0f} €</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">Coût main-d'œuvre</div>
            <div class="stat-value">{cout_mo_global:,.0f} €</div>
        </div>
    </div>
</div>

<div class="legend">
    <div class="legend-title">📋 Légende du tableau :</div>
    <ul style="margin: 5px 0; padding-left: 20px;">
        <li><b>{step_col}</b> : nom de l'étape de production (ex. "Assemblage aile gauche")</li>
        <li><b>Temps_Prevu_td</b> : temps prévu cumulé (format HH:MM:SS)</li>
        <li><b>Temps_prevu_heures</b> : temps prévu cumulé en heures</li>
        <li><b>Nb_personnes</b> : nombre de personnes ayant travaillé sur l'étape</li>
        <li><b>Nb_pieces</b> : nombre de pièces (occurrences) utilisées</li>
        <li><b>Cout_pieces</b> : coût total des pièces</li>
        <li><b>Cout_main_oeuvre</b> : coût total main-d'œuvre</li>
        <li><b>Cout_total</b> : coût global (pièces + main-d'œuvre) ⬅️ <i>Trié par ordre décroissant</i></li>
    </ul>
</div>
